...
const Sidebar = ({ onNavigate, currentPage = 'dashboard', onLogout, user }) => {
  // Get user display info
  const displayName = user?.nickname || user?.firstName || 'Guest';
  const membership = user?.role === 'admin' ? 'Administrator' : 'Member';
      ...
      React.createElement("div", null,
        ...
        React.createElement("div", { className: "flex items-center mb-10 pl-2" },
          ...
          React.createElement("div", null,
            ...
            React.createElement("p", { className: "text-xs text-gray-500 uppercase tracking-wider" }, "AI HEALTH COMPANION")
          )
        ),
        ...
        React.createElement("nav", { className: "space-y-2" },
          DASHBOARD_NAV_LINKS.map((link) => (
            React.createElement(NavLink, {
              key: link.name,
              href: "#", // Handle navigation via onClick
              ...
              // Fix: The NavLink component now accepts aria-current
      ...
      React.createElement("div", { className: "mt-auto border-t border-gray-200 pt-6 px-2" },
        React.createElement("div", { className: "flex items-center space-x-3" },
          ...
          React.createElement("div", null,
            ...
            React.createElement("p", { className: "text-xs text-gray-500" }, membership)
          )
        ),
