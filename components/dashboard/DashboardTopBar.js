...
const DashboardTopBar = ({ title, subtitle, showSearchBar = true, searchPlaceholder = "Search logs...", user, onLogout }) => {
  const [showProfileMenu, setShowProfileMenu] = useState(false);
  
  // Use actual user data or fallback
  const displayName = user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.nickname || user.email : 'User';
  const displayRole = user?.role || 'Member';
  const displayAvatar = user?.avatar;
  const initials = user ? `${user.firstName?.[0] || ''}${user.lastName?.[0] || ''}`.toUpperCase() : 'U';

  return (
    React.createElement("div", { className: "flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6 bg-white p-4 sm:p-5 rounded-xl shadow-sm border border-gray-100" },
      React.createElement("div", { className: "min-w-0" },
        React.createElement("h1", { className: "text-2xl sm:text-3xl font-bold text-gray-900 break-words" }, title),
        subtitle && React.createElement("p", { className: "text-sm sm:text-md text-gray-500 mt-0.5 break-words" }, subtitle)
      ),
      React.createElement("div", { className: "flex items-center justify-end w-full space-x-3 sm:space-x-4" },
        showSearchBar && (
          React.createElement("div", { className: "relative" },
            React.createElement("input", {
              type: "text",
              placeholder: searchPlaceholder,
              className: "pl-10 pr-4 py-2 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-200 w-full sm:w-64"
            }),
