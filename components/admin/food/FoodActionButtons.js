import React from 'react';

const FoodActionButtons = ({ foodId, onApprove, onReject, status }) => {
  const handleApproveClick = (e) => {
    e.stopPropagation();
    onApprove(foodId);
  };

  const handleRejectClick = (e) => {
    e.stopPropagation();
    onReject(foodId);
  };

  return (
    React.createElement("div", { className: "flex items-center justify-end space-x-3" },
      React.createElement("button", {
        onClick: handleApproveClick,
        className: `p-1.5 rounded-full ${status === 'approved' ? 'bg-green-100 text-green-700' : 'text-gray-400 hover:bg-green-50 hover:text-green-600'} transition-colors duration-200`,
        disabled: status === 'approved',
        "aria-label": "Approve food item"
      },
        React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "currentColor", className: "w-5 h-5" },
          React.createElement("path", { fillRule: "evenodd", d: "M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z", clipRule: "evenodd" })
        )
      ),
      React.createElement("button", {
        onClick: handleRejectClick,
        className: `p-1.5 rounded-full ${status === 'rejected' ? 'bg-red-100 text-red-700' : 'text-gray-400 hover:bg-red-50 hover:text-red-600'} transition-colors duration-200`,
        disabled: status === 'rejected',
        "aria-label": "Reject food item"
      },
        React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "currentColor", className: "w-5 h-5" },
          React.createElement("path", { fillRule: "evenodd", d: "M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zm-1.72 6.97a.75.75 0 10-1.06 1.06L10.94 12l-2.78 2.78a.75.75 0 101.06 1.06L12 13.06l2.78 2.78a.75.75 0 001.06-1.06L13.06 12l2.78-2.78a.75.75 0 00-1.06-1.06L12 10.94l-2.78-2.78z", clipRule: "evenodd" })
        )
      )
    )
  );
};

export default FoodActionButtons;